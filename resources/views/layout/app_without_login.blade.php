@php
  ...
  $setting = Setting::where('text_key','general_setting_login_logo')->first();
@endphp
...
<head>
  ...
  <title>@yield('title') - CW Offer</title>
...
<body class="hold-transition login-page">
    ...
    <div class="auth_container" id="contents" style="opacity: 0;">
      <div class="auth_logo text-center">
          <a href="{{url('/')}}"><img src="@if(!empty($setting->text_value)) {{ url('public/uploads/'.Config::get('constant.images_dirs.SETTING').'/'.$setting->text_value) }} @else {{ url('public/images/CityWorth_Homes_with_nmls-FB.png') }} @endif" alt="CW Offer Logo" class="login_logo img-fluid"></a>
      </div>
      @yield('content')
    </div>
