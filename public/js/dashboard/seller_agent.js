...
$(function() {
    ...
    $('#sub_admin_listing').on( 'processing.dt', function ( e, settings, processing ) {
          ...
          "deferRender": true,
          "ajax": {
              "url": base_url+"/admin/sellerAgents",
              "type": "POST",
              "dataType": 'json',
              "data": function ( d ) {
          ...
          columns:[
              ...
              { "data": "Action", sortable:!1,
                render: function (data, type, row) {
                  return'<a class="ajax_request" data-slug="admin/viewSellerAgent/'+row.user_id+'" href="'+base_url+'/admin/viewSellerAgent/'+row.user_id+'"><img src="'+imagepath+'/ic_eye_color.png"></a>\t\t\t\t\t\t<a  class="ajax_request" data-slug="admin/editSellerAgent/'+row.user_id+'" href="'+base_url+'/admin/editSellerAgent/'+row.user_id+'"><img src="'+imagepath+'/ic_mode_edit.png"></a>\t\t\t\t\t\t<a onclick="triggerDelete('+row.user_id+')" href="javascript:void(0)"><img src="'+imagepath+'/ic_delete.png"></a>'
                }
              },
        ],
    ...
    $("form[name='add-sub-admin-form']").validate({
      errorClass: "error_msg",
       rules: {
        ...
        $.ajax({
            url: base_url+'/admin/addSellerAgentPost',
            type: 'POST',
            processData: false,
            contentType: false,
            cache: false,              
            ...
            success: function(result)
            {
                if(result.status){
                  ...
                  $('li a[data-slug="admin/sellerAgents"]').trigger("click");
                }else{
...
function confirmDelete(){
  ...
  var cid = $('#did').val();
  $.ajax({
    url: base_url+'/admin/deleteSellerAgent',
    type: 'POST',
    dataType:'json',
    cache: false,              
    data: {'cid':cid},
...
function confirmStatus(cid){
  ...
  var cid = $('#did').val();
  $.ajax({
    url: base_url+'/admin/statusSellerAgent',
    type: 'POST',
    dataType:'json',
    cache: false,              
    data: {'cid':cid},
