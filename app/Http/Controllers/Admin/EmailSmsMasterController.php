...
class EmailSmsMasterController extends Controller
{
    /**
    ...
    public function getEmailTemplates(Request $request)
    {
       ...
       */
    	...
    	$template = new EmailSmsMaster;

    	if($filter){
    		$template = $template->where( 'title', 'LIKE', '%' . $filter . '%' );
    	}
    ...
    {
    	$data = EmailSmsMaster::where('email_sms_master_id',$id)->first();
    ...
    public function editEmailTemplatePost(Request $request)
    {
        ...
        try
        {
            $emailMaster = EmailSmsMaster::where('email_sms_master_id',$input['cid'])->first();
